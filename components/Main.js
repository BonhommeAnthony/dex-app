...
const Main = ({ web3, accounts, contracts }) => {
  ...
  const listenToTrades = (token) => {
    ...
    const listener = contracts.dex.events
      ...
      .on("data", (newTrade) => {
        if (tradeIds.has(newTrade.returnValues.orderId)) return;
        tradeIds.add(newTrade.returnValues.orderId);
  ...
  return (
    <>
      ...
      <SimpleGrid columns={[1, 1, 2, 3]} spacing={10}>
        ...
        {user.selectedToken.ticker !== "DAI" ? (
          <GridItem direction="column" colSpan={2}>
